...
public static Connection GetConnection()
    {
        ...
        try
        {
            ...
            String url = "jdbc:sqlserver://MICHEL ;databaseName=fábrica;user=sa;password=sa;";
   ...
   public static Connection GetResetearConnection()
    {
        String host=null;
        String pasw = null;
        String user = null;
        try{
        host= JOptionPane.showInputDialog(null, "Ingresa el nombre del host",null, JOptionPane.INFORMATION_MESSAGE);
        if(!host.equals(null)){
        user= JOptionPane.showInputDialog(null, "Ingresa el nombre del usuario",null, JOptionPane.INFORMATION_MESSAGE);
        if(!user.equals(null)){
        pasw= JOptionPane.showInputDialog(null, "Ingresa la contraseña",null, JOptionPane.INFORMATION_MESSAGE);
        }
        }
        }catch(Exception a){
           JOptionPane.showMessageDialog(null, "No se ingreso ningun valor.", null, JOptionPane.WARNING_MESSAGE);
        }
        
        try
        {
            Class.forName("com.microsoft.sqlserver.jdbc.SQLServerDriver");
            String url = "jdbc:sqlserver://"+host+";databaseName=fábrica;user="+user+";password="+pasw+";";
            conexion= DriverManager.getConnection(url);
            con = conexion.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,
                    ResultSet.CONCUR_UPDATABLE);
        }
    ...
    public ResultSet setBuscarRelacion(String tabla, String columna, int id, String tabla2, String columna2){
        try{
            String busqueda="select " +columna2+ " from "+tabla+ " join "+tabla2 +" where "+columna+ " = "+id;
            
            datos=con.executeQuery(busqueda);
    ...
    public ResultSet setBusqueda(String tabla, String columna, int id){
        try{
            String busqueda="select * from "+tabla+" where "+columna+ " = "+id;
    ...
    public boolean setEliminarRegistro(String tabla, String columna, int id,String tabla2, String columna2){
        ...
        try{
            String busqueda="use fábrica; delete from "+tabla+" where "+columna+ " = "+id;
            datos=con.executeQuery(busqueda);
            JOptionPane.showMessageDialog(null, "Se elimino Exitosamente", tabla+ "   "+id, JOptionPane.PLAIN_MESSAGE);
            e=false;
    ...
    public void getUpdateEmp(int id){
        try{
            String UpdEmp="update empleados set "
                    + "nombre = '"+this.nombre+
            ...
            con.executeUpdate(UpdEmp);
            JOptionPane.showMessageDialog(null, "ACTUALIZACION COMPLETA", "Actualizacion de BD", JOptionPane.INFORMATION_MESSAGE);
                //Icono de actualizar BD
        }catch(Exception a){
    ...
    public ResultSet getBusquedaEmp(String orden, String ordenAscDesc){
        String Consulta;
        try{
            Consulta="select * from empleados order by "+orden+" "+ordenAscDesc;
    ...
    public void getUpdateInsumos(int cod){
       try {
           ...
           preparedStatement.setInt(8,this.códigoInsumo);
    ...
    public ResultSet getInsumos(String orden, String ordenAscDesc){
        try{
            String Consulta="select * from insumos order by "+orden+" "+ordenAscDesc;
            datos=con.executeQuery(Consulta);
            return datos;
        }catch(Exception a){
            JOptionPane.showMessageDialog(null,"ERROR DE BASE DE DATOS  ==  "+a.getMessage(), "ERROR", JOptionPane.ERROR_MESSAGE);
   ...
   //GET NUEVA COMPRA
    ...
    public ResultSet getCalcularTotal( int a){
        try{
            String q= "select costo from insumos where codigo = "+ a;
            datos=con.executeQuery(q);
            return datos;
        }catch(Exception ag){
    ...
    public ResultSet getCompras(String orden, String ordenAscDesc){
        try{
            String Consulta="select * from compras order by "+orden+" "+ordenAscDesc;
            datos=con.executeQuery(Consulta);
            return datos;
        }catch(Exception a){
            JOptionPane.showMessageDialog(null,"ERROR DE BASE DE DATOS  ==  "+a.getMessage(), "ERROR", JOptionPane.ERROR_MESSAGE);
    ...
    public ResultSet getCliente(String orden, String ordenAscDesc){
        try{
            String Consulta="select * from clientes order by "+orden+" "+ordenAscDesc;
            datos=con.executeQuery(Consulta);
            return datos;
        }catch(Exception a){
            JOptionPane.showMessageDialog(null,"ERROR DE BASE DE DATOS  ==  "+a.getMessage(), "ERROR", JOptionPane.ERROR_MESSAGE);
    ...
    public ResultSet getProductos(String orden, String ordenAscDesc){
        try{
            String Consulta="select * from inventarioProductos order by "+orden+" "+ordenAscDesc;
            datos=con.executeQuery(Consulta);
            return datos;
        }catch(Exception a){
            JOptionPane.showMessageDialog(null,"ERROR DE BASE DE DATOS  ==  "+a.getMessage(), "ERROR", JOptionPane.ERROR_MESSAGE);
    ...
    public ResultSet getAsistenciaJust(String orden, String ordenAscDesc){
        try{
            String Consulta="select nombre,apellidos,listaAsistencia.fecha,horaLlegada,listaAsistencia.horaSalida, numJustificante,motivo,justificantes.horaSalida\n" +
"                    from listaAsistencia, empleados,justificantes\n" +
"                    where (empleados.NúmeroEmpleado=listaAsistencia.NúmeroEmpleado \n" +
"                    and justificantes.numJustificante=listaAsistencia.justificante)"
    ...
    public ResultSet getJustificantes(String orden, String ordenAscDesc){
        try{
            String Consulta="select nombre,apellidos,fecha,motivo,horaSalida"
    ...
    public void getVentas(){
       String SqlQuery= "insert into ventas("
                ...
                this.codProdVenta+"','"+
                this.cant+"',"+
                this.numClientVenta+"','"+this.totalVenta+"','"+this.tipoPagoVenta+"','"+this.vencPagoVenta+"','"+this.pagaréVenta+
                ");";
        String DATOS = SqlQuery;
       try {
           con.executeUpdate(DATOS);
    ...
    public ResultSet getBusquedaVentas(String orden, String ordenAscDesc){
        try{
            String Consulta="select * from ventas order by "+orden+" "+ordenAscDesc;
            datos=con.executeQuery(Consulta);
            return datos;
        }catch(Exception a){
            JOptionPane.showMessageDialog(null,"ERROR DE BASE DE DATOS  ==  "+a.getMessage(), "ERROR", JOptionPane.ERROR_MESSAGE);
